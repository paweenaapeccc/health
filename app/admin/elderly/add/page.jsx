...
export default function AddElderlyMemberPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    province: '',
    // ✅ ใช้คอลัมน์เดียว
    latlong: '' // ตัวอย่างค่าที่กรอก: "14.988756,103.100897"
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/elderly', {
        ...
        body: JSON.stringify({
          ...formData,
          phone: formData.phoneNumber, // DB ใช้ phonNumber
        })
      })
        ...
        <ClientOnly>
          <form onSubmit={handleSubmit} className="rounded-2xl bg-white shadow-md ring-1 ring-slate-100 p-6 md:p-8 space-y-8" autoComplete="off">
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:items-center gap-3 pt-2">
              <button type="button" onClick={() => router.push('/member/elderly')}
